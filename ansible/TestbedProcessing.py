...
parser.add_argument('-i', help='a file for the testbed processing script',
                    nargs="?", default="testbed-new.yaml")
parser.add_argument(
    '-basedir', help='base directory to find the files, points to /sonic-mgmt/ansible', default="")
parser.add_argument(
    '-backupdir',
    help='backup directory to store files,  points to /sonic-mgmt/ansible/backup', nargs="?", default="backup")
parser.add_argument(
    '-yaml', help='make lab and veos file in YAML format', action='store_true')
...
veos_file = "veos"
# the number of host_var files vary. therefore, backup process creates a list of all files under host_vars folder,
# iterates through the list, and copies them to backup directory

# Backup List
# backupList does not encompass host_var files because the number of host_var files vary.
# therefore, backup process creates a list of all files under host_vars folder, iterates through the list,
# and copies them to backup directory
backupList = []
...
timestamp = str(now.month) + "_" + str(now.day) + "_" + str(now.year) + \
    "_" + str(now.hour) + str(now.minute) + "_" + str(now.second)
# create folder in backup directory labeled with the current timestamp
os.makedirs(args.backupdir + "/" + timestamp)
# create files folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/files")
# create host_vars folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/host_vars")
# create group_vars folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/group_vars")
# create group_vars/eos folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/group_vars/eos")
# create group_vars/fanout folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/group_vars/fanout")
# create group_vars/lab folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/group_vars/lab")
# create group_vars/vm_host folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/group_vars/vm_host")
# create vars folder under the timestamped folder
os.makedirs(args.backupdir + "/" + timestamp + "/vars")
    ...
    try:
        with open(csv_file, "w") as f:
            ...
            for device, deviceDetails in topology.items():
                hostname = device
                managementIP = str(deviceDetails.get(
                    "ansible").get("ansible_host"))
                hwsku = deviceDetails.get("hwsku")
                # DevSonic, server, FanoutRoot etc
                devType = deviceDetails.get("device_type")
                # supervisor, Linecard etc
                cardType = deviceDetails.get("card_type")
                # catch empty values
                if not managementIP:
                    managementIP = ""
                if not hwsku:
                ...
                row = hostname + "," + managementIP + "," + \
                    hwsku + "," + devType + "," + cardType
                f.write(row + "\n")
    except IOError:
...
def makeLabSecrets(data, outfile):
    ...
    for key, value in devices.items():
        if "server" in value.get("device_type").lower():
            result.update({"ansible_ssh_pass": value.get(
                "ansible").get("ansible_ssh_pass")})
            result.update({"ansible_become_pass": value.get(
                "ansible").get("ansible_become_pass")})
            result.update({"sonicadmin_user": value.get(
                "ansible").get("sonicadmin_user")})
            result.update({"sonicadmin_password": value.get(
                    ...
                    if "ptf" in key:
                        ...
                        try:  # get num_fabric_asics
                            ...
                            if num_fabric_asics is not None:
                                entry += "\tnum_fabric_asics=" + \
                                    str(num_fabric_asics)
                        except AttributeError:
                        ...
                        try:  # get switch_type
                            ...
                            if switch_type is not None:
                                ...
                                if switch_type == 'voq' and card_type != 'supervisor':
                                    ...
                                    voq_inband_ipv6 = dev.get(
                                        "voq_inband_ipv6")           # voq_inband_ipv6
                                    voq_inband_intf = dev.get(
                                        "voq_inband_intf")           # voq_inband_intf
                                    voq_inband_type = dev.get(
                                        "voq_inband_type")           # voq_inband_type
                                    # max cores
                                    max_cores = dev.get("max_cores")
                                    # loopback4096_ip
                                    lo4096_ip = dev.get("loopback4096_ip")
                                    # loopback4096_ipv6
                                    lo4096_ipv6 = dev.get("loopback4096_ipv6")
                                    # number of cores per asic - to be used in calculating the switchids,
                                    # assuming to be the same for all linecards
                                    num_cores_per_asic = dev.get(
                                        "num_cores_per_asic", 1)
...
@:parameter data - reads from either veos-groups, this helps separate the function into 3 components;
                   children, host, vars
...
def makeVeosYAML(data, veos, devices, outfile):
    ...
    if 'servers' in group:
        serversDict.update(
            {'vars': group['servers'].get("vars"), 'children': {}})
...
def main():
    ...
    for file in backupList:
        try:
            copyfile(args.basedir + file, args.backupdir +
                     "/" + timestamp + "/" + file)
    ...
    # dictionary contains information about veos
    veos = dict()
    generateDictionary(doc, veos, "veos")                   # load veos
    # dictionary contains information about testbed (ptf)
    testbed = dict()
    generateDictionary(doc, testbed, "testbed")             # load testbed
    # dictionary contains information about toplogy
    topology = dict()
    generateDictionary(doc, topology, "topology")           # load topology
    # dictionary contains information about host_vars
    host_vars = dict()
    generateDictionary(doc, host_vars, "host_vars")         # load host_vars
    # dictionary contains information about veos_groups
    veos_groups = dict()
    generateDictionary(doc, veos_groups, "veos_groups")     # load veos_groups
    # dictionary contains information about device_groups
    device_groups = dict()
    # load device_groups
    generateDictionary(doc, device_groups, "device_groups")
    # dictionary contains information about docker_registry
    ...
    # Generate sonic_lab_links.csv (TOPOLOGY)
    makeSonicLabLinks(topology, args.basedir + labLinks_file)
    ...
    # Generate eos\creds.yml (CREDS)
    makeEOSCreds(veos, args.basedir + eosCred_file)
    print("\tCREATING FANOUT/SECRETS: " + args.basedir + fanoutSecrets_file)
    # Generate fanout\secrets.yml (SECRETS)
    makeFanoutSecrets(devices, args.basedir + fanoutSecrets_file)
    ...
    if args.yaml:
        # Generate veos in YAML file format(VEOS)
        makeVeosYAML(veos_groups, veos, devices, args.basedir + veos_file)
    else:
        # Generate veos in INI file format(VEOS)
        makeVeos(veos_groups, veos, devices, args.basedir + veos_file)
